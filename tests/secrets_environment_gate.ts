...
const textualFiles=tracked.filter(file=>!file.endsWith('.lock')&&!/\.(png|jpg|jpeg|gif|webp|ico|pdf|woff|woff2|ttf|eot|zip|gz|tar)$/i.test(file));
for(const file of textualFiles){const text=readFileSync(join(root,file),'utf8');const credentialUrls=text.match(credentialUrl)||[];const unexpectedCredentials=credentialUrls.filter(value=>value!==allowedLocalDatabaseUrl&&!/[<>]/.test(value));assert.equal(unexpectedCredentials.length,0,'hard-coded non-local credential URL in '+file);assert.doesNotMatch(text,realCredentialMarker,'credential-like token in '+file);}
