...
for(const file of tracked.filter(file=>!file.endsWith('.lock'))){const text=readFileSync(join(root,file),'utf8');if(file==='.env.example')continue;const credentialUrls=text.match(credentialUrl)||[];const nonLocalCredentials=credentialUrls.filter(value=>!/^postgres(?:ql)?:\/\/postgres:postgres@localhost(?::5432)?\//i.test(value));assert.equal(nonLocalCredentials.length,0,'hard-coded non-local credential URL in '+file);assert.doesNotMatch(text,realCredentialMarker,'credential-like token in '+file);}
