 ...
 return <AppShell><div className="mx-auto max-w-3xl">
  ...
  <div className="panel mt-6 p-6">
   ...
   <div className="mt-3 grid gap-3 md:grid-cols-3">
    {levels.map(([id,label,desc])=><button key={id} type="button" onClick={()=>setLevel(id)} className={`rounded-lg border p-4 text-left ${level===id?'border-emerald-500/60 bg-emerald-950/20':'border-white/10 bg-slate-950/40'}`}>
      <span className="block text-sm font-medium">{label}</span><span className="mt-2 block text-xs leading-5 text-slate-500">{desc}</span>
    </button>)}
   </div>
   <p className="mt-7 text-xs font-medium uppercase tracking-[0.14em] text-slate-500">2. Number of questions</p>
   <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
    {counts.map(n=><button key={n} type="button" onClick={()=>setCount(n)} className={`rounded-lg border p-4 text-center ${count===n?'border-emerald-500/60 bg-emerald-950/20':'border-white/10 bg-slate-950/40'}`}>
      <span className="block text-lg font-semibold">{n}</span><span className="text-xs text-slate-500">questions</span>
    </button>)}
   </div>
   <div className="mt-6 rounded-lg border border-white/10 bg-slate-950/40 p-4">
    <div className="flex items-center justify-between"><span className="text-sm font-medium text-slate-300">Assessment duration</span><span className="text-sm font-semibold text-emerald-300">{minutes} minutes</span></div>
