     ...
     <div className="h-[calc(100%-2.5rem)] overflow-auto p-4 font-mono text-xs">
      ...
      {consoleTab==='output'&&<div>
       ...
       {runResult&&<div>
        ...
        {runResult.cases?.[0]?.actualOutputPreview&&<div className="rounded-lg border border-white/10 overflow-auto"><table className="w-full text-left text-[11px]"><thead><tr>{runResult.cases[0].actualOutputPreview.columns.map((x:string)=><th key={x} className="border-b border-white/10 px-3 py-2 text-slate-500">{x}</th>)}</tr></thead><tbody>{runResult.cases[0].actualOutputPreview.rows.map((row:any,i:number)=><tr key={i}>{runResult.cases[0].actualOutputPreview.columns.map((x:string)=><td key={x} className="border-b border-white/5 px-3 py-2 text-slate-300">{String(row[x]??'NULL')}</td>)}</tr>)}</tbody></table></div>}
      ...
      {consoleTab==='tests'&&<div>
       ...
       {(testResult?.cases||testResult?.testCases||[]).map((t:any,i:number)=><div key={t.id||i} className="mb-3 rounded-lg border border-white/10 p-3">
        <div className="flex justify-between"><span className="text-slate-300">Case {i+1} · {t.name}</span><span className={t.status==='AC'?'text-emerald-300':'text-rose-300'}>{t.status}</span></div>
        {t.diagnosticAdvice&&<div className="mt-2 rounded border border-rose-500/20 bg-rose-500/5 p-2 text-rose-300">Diagnosis: {t.diagnosticAdvice}</div>}
        {t.diff&&<div className="mt-2 text-[11px] text-slate-400">Mismatch: row {t.diff.rowIndex}, column <span className="text-rose-300">{t.diff.columnName}</span> · expected <span className="text-emerald-300">{t.diff.expectedValue}</span> · actual <span className="text-rose-300">{t.diff.actualValue}</span></div>}
        {t.isPublic&&t.expectedOutputPreview&&t.actualOutputPreview&&<div className="mt-3 grid min-w-[560px] grid-cols-2 gap-3">
         <div><div className="mb-1 text-[10px] uppercase tracking-wider text-slate-600">Expected</div><div className="overflow-auto rounded border border-white/5"><table className="w-full text-left text-[10px]"><thead><tr>{t.expectedOutputPreview.columns.map((x:string)=><th key={x} className="px-2 py-1 text-slate-600">{x}</th>)}</tr></thead><tbody>{t.expectedOutputPreview.rows.map((row:any,j:number)=><tr key={j}>{t.expectedOutputPreview.columns.map((x:string)=><td key={x} className="px-2 py-1 text-slate-400">{String(row[x]??'NULL')}</td>)}</tr>)}</tbody></table></div></div>
         <div><div className="mb-1 text-[10px] uppercase tracking-wider text-slate-600">Your output</div><div className="overflow-auto rounded border border-white/5"><table className="w-full text-left text-[10px]"><thead><tr>{t.actualOutputPreview.columns.map((x:string)=><th key={x} className="px-2 py-1 text-slate-600">{x}</th>)}</tr></thead><tbody>{t.actualOutputPreview.rows.map((row:any,j:number)=><tr key={j}>{t.actualOutputPreview.columns.map((x:string)=><td key={x} className={'px-2 py-1 '+(t.diff&&t.diff.rowIndex===j+1&&t.diff.columnName===x?'bg-rose-950 text-rose-300 font-semibold':'text-slate-400')}>{String(row[x]??'NULL')}</td>)}</tr>)}</tbody></table></div></div>
        </div>}
        {!t.isPublic&&<div className="mt-2 text-[11px] text-slate-600">Hidden test details remain masked; only pass/fail is exposed.</div>}
       </div>)}
      </div>}
