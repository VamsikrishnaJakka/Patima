...
export async function GET(){
 try{const s=await requireCandidate();const data=await withAuthenticatedClient(s,async(session,client)=>{const r=await client.query(`SELECT visibility,accepting_contact_requests,peer_visibility FROM candidate_visibility_settings WHERE user_id=$1`,[session.userId]);return r.rows[0]||{visibility:'PRIVATE',accepting_contact_requests:true,peer_visibility:false};});return NextResponse.json(data,{headers:{'Cache-Control':'no-store'}});}catch{ return NextResponse.json({error:'Unauthorized'},{status:401}); }
}

export async function POST(req:Request){
 try{
  const s=await requireCandidate(); const body=await req.json();
