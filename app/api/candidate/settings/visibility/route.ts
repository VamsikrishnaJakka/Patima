...
export async function POST(req:Request){
 try{
  ...
  if(typeof body.accepting_contact_requests!=='boolean'||typeof body.peer_visibility!=='boolean')return NextResponse.json({error:'Privacy flags must be boolean values'},{status:400});
  const accepting=body.accepting_contact_requests;
  const peers=body.peer_visibility;
