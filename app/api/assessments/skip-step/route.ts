...
export async function POST(request:Request){
 try{
  ...
  return await withSessionClient(s.userId,async(c)=>{
   ...
   const next=await allocateNextQuestion(c,{sessionId,userId:s.userId,expectedVariantId:variantId,submittedCode:'[SKIPPED]',durationSeconds:0,verificationReport:{verdict:'SKIPPED',publicTestsPassed:0,publicTestsTotal:0,hiddenTestsPassed:0,hiddenTestsTotal:0,executionTimeMs:0,peakMemoryKb:null,executionDigest:crypto.createHash('sha256').update(sessionId+variantId).digest('hex'),isCorrect:null}});
