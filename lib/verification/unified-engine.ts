...
import{EngineMode,UnifiedCase,UnifiedEngineResponse,DiffLocation}from'./unified-types';
...
export async function executeUnifiedEngine(req:{mode:EngineMode;sql:string;variant:Variant}):Promise<UnifiedEngineResponse>{
 const {mode,sql,variant}=req,start=performance.now();
 ...
 return {mode,verdict,runtimeMs:Number((performance.now()-start).toFixed(2)),summary:{passed,total:cases.length,publicPassed:pub.filter(x=>x.status==='AC').length,publicTotal:pub.length,hiddenPassed:hid.filter(x=>x.status==='AC').length,hiddenTotal:hid.length},cases,sqlAnalysis:{singleStatement:true,allowedTablesOnly:true,detectedClauses:detectClauses(sql),performanceObservation:first?'See failing case diagnosis.':undefined},executionDigest:crypto.createHash('sha256').update(sql+JSON.stringify(cases)).digest('hex'),nextStepAvailable:mode==='SUBMIT'&&all};
}
